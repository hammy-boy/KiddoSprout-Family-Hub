(function () {
  ...
  const liveDownloadFields = document.getElementById("blocker-live-download-fields");
  const demoDownloadSummary = document.getElementById("blocker-demo-download-summary");
  ...
  function renderPlatformChoice() {
    ...
    downloadPlatformLabel.textContent = READ_ONLY_DEMO
      ? `Download ${copy.name} demo setup guide`
  ...
  function demoGuideFilename() {
    const artifactId = selectedArtifact();
    if (artifactId === "windows-arm64") return "KiddoSprout-Windows-ARM64-Demo-Setup-Guide.txt";
    if (artifactId === "windows") return "KiddoSprout-Windows-x64-Demo-Setup-Guide.txt";
    return "KiddoSprout-Mac-Demo-Setup-Guide.txt";
  }

  function demoGuideText() {
    const copy = selectedPlatformCopy();
    const platformSteps = copy.family === "windows"
      ? [
          `Confirm whether the computer needs the ${copy.name} package.`,
          "Keep Windows Security and SmartScreen switched on.",
          "A parent should review the protected download and its SHA-256 checksum before opening it.",
          "Use a non-administrator child account for everyday use.",
          "Test website and installed-game blocking with the computer owner's permission."
        ]
      : [
          "Use a Standard child account while the parent keeps the administrator account.",
          "Keep Gatekeeper switched on; never disable macOS security to open an app.",
          "A parent should review the protected download and its SHA-256 checksum before opening it.",
          "Move only the reviewed companion into Applications.",
          "Test website and installed-game blocking with the computer owner's permission."
        ];
    return [
      "KiddoSprout Demo Setup Guide",
      "============================",
      "",
      "DEMO FILE — TEXT ONLY",
      `Selected preview: ${copy.name}`,
      "",
      "This harmless sample contains no blocker app, installer, browser extension, PIN, password, or executable code.",
      "It cannot install, run, block, monitor, or change anything on this computer.",
      "",
      "Preview checklist",
      ...platformSteps.map((step, index) => `${index + 1}. ${step}`),
      "",
      "Real companion downloads remain behind a verified KiddoSprout parent account.",
      "KiddoSprout family calls and local emergency services should never be replaced by this guide."
    ].join("\n");
  }

  function downloadDemoGuide() {
    if (downloadInProgress) return;
    const copy = selectedPlatformCopy();
    const filename = demoGuideFilename();
    downloadInProgress = true;
    downloadButton.disabled = true;
    setPlatformControlsDisabled(true);
    form.setAttribute("aria-busy", "true");
    downloadPlatformIcon.textContent = "⌛";
    downloadPlatformLabel.textContent = `Preparing ${copy.name} text guide…`;
    try {
      const blob = new Blob([demoGuideText()], { type: "text/plain;charset=utf-8" });
      triggerDownload(blob, filename);
      setStatus(`Downloaded ${filename}. It is a harmless text guide; no app was installed or run.`, "success");
    } catch (error) {
      setStatus("The demo guide could not be saved in this browser. Nothing was installed or run.", "error");
    } finally {
  ...
  function initializePublicPreview() {
    ...
    buildLabel.textContent = "Demo · Harmless setup guide";
    downloadTitle.textContent = "Try the Mac or Windows setup flow";
    intro.textContent = "Choose a computer, review its steps, then download a plain-text demo checklist. No installer or app is included.";
    previewNote.hidden = false;
    form.dataset.downloadMode = "demo-guide";
    liveDownloadFields.hidden = true;
    demoDownloadSummary.hidden = false;
    ...
    const renderPublicSelection = () => {
      renderPlatformChoice();
      const copy = selectedPlatformCopy();
      setStatus(`Ready to download the harmless ${copy.name} text setup guide. No app or installer is included.`, "preview");
    };
    form.addEventListener("submit", (event) => {
      event.preventDefault();
      downloadDemoGuide();
    });
    bindPlatformChoices(renderPublicSelection);
