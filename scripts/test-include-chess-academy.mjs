...
const developmentChessRoot=fileURLToPath(new URL('../games/chess-academy/',import.meta.url));
...
test('Pages artifact retains the exact published chess files and internal link without replacing unrelated content',async()=>{
 ...
 try{
  ...
  for(const file of CHESS_FILES){
   const publishedSource=await readFile(join(sourceRoot,'games/chess-academy',file));
   assert.deepEqual(await readFile(join(outputRoot,'games/chess-academy',file)),publishedSource);
   assert.deepEqual(await readFile(join(developmentChessRoot,file)),publishedSource,`Published and tested Chess copies differ: ${file}`);
  }
