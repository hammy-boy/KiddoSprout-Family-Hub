(()=>{
 ...
 let distance=0,progress=0,jump=0,velocity=0,running=false,paused=false,op='multiply',objects=[],onEnd=()=>{},onCoin=()=>{},last=performance.now(),travel=0,frame=0;
 const reduced=window.matchMedia?.('(prefers-reduced-motion: reduce)').matches===true;
 function schedule(){if(!frame)frame=requestAnimationFrame(tick)}
 function begin(operation,round,done,coin){op=operation;travel=0;progress=0;jump=0;velocity=0;running=true;paused=false;onEnd=done;onCoin=coin;objects=[{x:220,type:'coin',taken:false},{x:330,type:'log',taken:false},{x:445,type:'coin',taken:false}];canvas.dataset.phase='running';canvas.dataset.checkpoint=round+1;last=performance.now();draw();if(reduced){running=false;canvas.dataset.phase='question';queueMicrotask(onEnd);return}schedule()}
